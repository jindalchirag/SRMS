...
<?php
$class=$_POST['class'];
$sql = "SELECT * from tblstudents join tblclasses on tblstudents.ClassId=tblclasses.id where tblstudents.ClassId=$class";
$query = $dbh->prepare($sql);
$query->execute();
...
<div class="form-group">
                                                    ...
                                                    <div  id="subject">
                                                        <?php
                                                        $class=$_POST['class'];
$stmt = $dbh->prepare("SELECT tblsubjects.SubjectName,tblsubjects.id FROM tblsubjectcombination join  tblsubjects on  tblsubjects.id=tblsubjectcombination.SubjectId WHERE tblsubjectcombination.ClassId=$class order by tblsubjects.SubjectName");
 $stmt->execute(array(':cid' => $class));
  $sid1=array();
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
array_push($sid1,$row['SubjectName']);
   } 
   for($i=0;$i<count($sid1);$i++){
      $s=$sid1[$i];
                                                        
                                                     echo '<input type="text" class="form-control" placeholder="<? php echo $s;?>" name="marks[]">';
                                                        }
                                                        ?>
                                                    </div>
                                                        </div>
